...
/// Reverb
double wet_rev = 0.8, dry_rev = 0.1,rev_time = 0.4,rev_damping = 0.2;
...
int hammond_initialize()
{
    ...
    perc_time[0] = trans*buf_size + std::floor(0.5*sample_rate);
    perc_time[1] = trans*buf_size + std::floor(1.0*sample_rate);
    perc_time[2] = trans*buf_size + std::floor(2.5*sample_rate);
    perc_level[0] = 0.26;
    perc_level[1] = 0.07;
    perc_level[2] = 0.0;
...
inline int set_target_levels()
{
    ...
    for(auto it = keyboard.begin(); it != keyboard.end(); it++)
    {
        ...
        if(perc)
        {
        	wheel_perc[wirematrix[*it][perc] -1] += 2;
        	wheel_perc[crosstalk(wirematrix[*it][perc] -1)] += xtalk*2;
        }
    }
    // Estimate RMS from Tonewheel Target Levels and set Target Gain Coefficient **** To Be Replaced by a Real Compressor ****
...
inline double get_percussion()
{
	if(perc_counter > perc_time[2])
		p =  0.0;
	else if(perc_counter > perc_time[1])
		{p = perc_level[1]/(perc_time[2]-perc_time[1]);perc_counter++;}
	else if(perc_counter > perc_time[0])
		{p -= (perc_level[0]-perc_level[1])/(perc_time[1]-perc_time[0]);perc_counter++;}
	else if(perc_counter > trans*buf_size )
		{p -= (1.0 - perc_level[0])/(perc_time[0]-trans*buf_size);perc_counter++;}
	else if(perc_counter > 0)
		{p += 1.0/(trans*buf_size );perc_counter++;}
	else p = 0.0;
	//std::cout<<p<<" ";
	return p;
}
...
int main(int argc, char **argv)
{
    ...
    while((c = getopt_long(argc,argv,"s:b:d:x:c:V:t:f:T:r:D:W:hM:A:",opts,NULL)) != -1)
    {
        switch (c)
        {
            case 's' :
